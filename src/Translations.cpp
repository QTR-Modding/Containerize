...
namespace {
	void LoadTranslationSection(const rapidjson::Value& sectionObj, const std::map<std::string, std::string*>& section)
	{
...
bool Translations::LoadTranslations(const std::string& lang)
{
	...
	const std::filesystem::path path = std::filesystem::path(translations_folder) / 
                                   (mod_name + "_" + lang + ".json");
