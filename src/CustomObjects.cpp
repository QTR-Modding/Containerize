...
Source::Source(const std::uint32_t id, std::string id_str, const float capacity, const float cs, const bool a_transfer_on_use):
    weight_ratio(1 - cs), capacity(capacity), transfer_on_use(a_transfer_on_use), formid(id), editorid(std::move(id_str)) {
    if (!formid) {
