...
RE::BSEventNotifyControl EventSink::ProcessEvent(const RE::TESFurnitureEvent* event,
    ...
    if (!furniture_entered && event->type == RE::TESFurnitureEvent::FurnitureEventType::kExit)
        return RE::BSEventNotifyControl::kContinue;
    if (event->targetFurniture->GetBaseObject()->formType.underlying() != 40) return
    ...
    if (const auto bench_type = static_cast<std::uint8_t>(bench->workBenchData.benchType.get());
        bench_type != 2 && bench_type != 3 && bench_type != 7) return RE::BSEventNotifyControl::kContinue;
