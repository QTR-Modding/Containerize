...
class DynamicFormTracker final : public DFSaveLoadData {
    ...
    [[nodiscard]] static bool underlying_check(const RE::TESForm* underlying, const RE::TESForm* derivative) {
        if (underlying->GetFormType() != derivative->GetFormType()) {
            logger::trace(
                "Form types do not match: {} vs {}, ID: {:x} vs {:x}", RE::FormTypeToString(underlying->GetFormType()),
                RE::FormTypeToString(derivative->GetFormType()), underlying->GetFormID(), derivative->GetFormID());
...
public:
    ...
    bool IsActive(const FormID a_formid) const { return active_forms.contains(a_formid); }

    bool IsProtected(const FormID a_formid) const { return protected_forms.contains(a_formid); }
    ...
    FormID Fetch(const FormID baseFormID, const std::string baseEditorID, const std::optional<uint32_t> customID) {
    ...
    void Reserve(const FormID baseID, const std::string& baseEditorID, const FormID dynamic_formid) {
        ...
        const auto base_form = FormReader::GetFormByID(baseID, baseEditorID);
        if (!base_form) {
    ...
    size_t GetNDeleted() const { return deleted_forms.size(); }
            ...
            if (const auto* act_eff = *it) {
                if (const auto act_eff_formid = act_eff->spell->GetFormID(); active_forms.contains(act_eff_formid)) {
                    if (act_effs_temp.contains(act_eff_formid))
                        logger::warn("Active effect already exists in act effs.");
                    else
                        n_act_effs++;
        ...
        for (const auto& [base_pair, dyn_formset] : forms) {
            ...
            for (const auto dyn_formid : dyn_formset) {
                if (!IsActive(dyn_formid) && !IsProtected(dyn_formid))
                    logger::info("Inactive form {:x} found in forms set.", dyn_formid);
    ...
    void ReceiveData() {
        ...
        for (const auto& [lhs, rhs] : m_Data) {
            ...
            if (!temp_form)
                logger::critical("Failed to get base form.");
            else
            ...
            for (const auto& [dyn_formid, custom_id, act_eff_elpsd] : rhs) {
                ...
                if (const auto dyn_form = RE::TESForm::LookupByID(dyn_formid); !dyn_form) {
                    ...
                    if ([[maybe_unused]] const auto dyn_form_ref =
                        RE::TESForm::LookupByID<RE::TESObjectREFR>(dyn_formid)) {
    ...
    void ApplyMissingActiveEffects() {
        ...
        for (auto& [baseFormid, dynamicFormid, elapsed, customid] : act_effs) {
            ...
            const auto dyn_formid =
                GetByCustomID(custom_id, baseFormid, clib_util::editorID::get_editorID(base_mg_item));
            if (!dyn_formid) {
