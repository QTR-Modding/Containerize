...
namespace QuickLoot::API
{
	...
	public:
		...
		static bool Init(const char* plugin)
		{
			...
			_registerLinkedContainer = reinterpret_cast<RegisterLinkedContainerProc>(GetProcAddress(dllHandle, "QuickLootRegisterLinkedContainer"));
		...
		static void RegisterLinkedContainer(RE::FormID refID, RE::FormID containerRefID)
		{
			if (_registerLinkedContainer) {
				_registerLinkedContainer(refID, containerRefID);
			}
		}
	...
	private:
		...
		using RegisterLinkedContainerProc = void (*)(std::uint32_t, std::uint32_t);
		static inline RegisterLinkedContainerProc _registerLinkedContainer = nullptr;
	};
}
